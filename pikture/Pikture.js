...
function SaveAsImg(imageType, name){
    ...
    switch(imageType){
        case 0:
            pic.download = name.value.toString() +".png";
            pic.href = getCanvas().toDataURL("image/png").replace("image/png", "image/octet-stream");
            pic.click();
            break;
        case 1:
            pic.download = name.value.toString() +".jpeg";
            pic.href = getCanvas().toDataURL("image/png").replace("image/jpeg", "image/octet-stream");
            pic.click();
            break;
        case 2:
            pic.download = name.value.toString() +".tiff";
            pic.href = getCanvas().toDataURL("image/png").replace("image/tiff", "image/octet-stream");
            pic.click();
            break;
        case 3:
            pic.download = name.value.toString() +".gif";
            pic.href = getCanvas().toDataURL("image/png").replace("image/gif", "image/octet-stream");
            pic.click();
            break;
        case 4:
            pic.download = name.value.toString() +".bmp";
            pic.href = getCanvas().toDataURL("image/png").replace("image/bmp", "image/octet-stream");
            pic.click();
            break;
        default:
...
window.onload = function() {
    ...
    console.log("loaded");
    var name = "untitled";
    var nameOfFile = document.getElementById ("picName").value = name;
